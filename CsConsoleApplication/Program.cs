...
namespace CsConsoleApplication
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            AdventOfCode4.Run2(false);
        }

        public static void CalcHash()
        {
            uint numBuckets = 5;
            ...
            using (var fileStream = File.OpenRead(@"C:\_in\hash obj_id\objid.tsv"))
            using (var streamReader = new StreamReader(fileStream, Encoding.UTF8, true, BufferSize))
            {
                String line;
                while ((line = streamReader.ReadLine()) != null)
                {
                    var crcVal = crc32(line);

                    buckets[crcVal % numBuckets]++;
                }

                for (uint i = 0; i < numBuckets; i++)
                    Console.WriteLine("Bucket " + i + ": " + buckets[i]);
